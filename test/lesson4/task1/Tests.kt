...
class Tests {
    ...
    @Tag("Normal")
    fun factorize() {
        assertEquals(listOf(3, 3, 3, 11), factorize(297))
    ...
    @Tag("Hard")
    fun roman() {
        assertEquals("MMMMMMMMMMMMMMMMMMMCLXXII", roman(19172))
    ...
    @Tag("Impossible")
    fun russian() {
        assertEquals("четыреста сорок четыре тысячи семьсот семнадцать", russian(444717))
        assertEquals("девятьсот тридцать девять тысяч девятьсот восемьдесят пять", russian(939985))
